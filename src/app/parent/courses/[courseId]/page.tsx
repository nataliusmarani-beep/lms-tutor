...
export default async function ParentCoursePage({ params }: { params: { courseId: string } }) {
  ...
  const { data: course } = await supabase
    .from("courses").select("id, title, title_id, icon, icon_url, description, description_id, created_by").eq("id", params.courseId).single();
  if (!course) notFound();
              ...
              {overallPct < 100 ? (
                ...
                <div className="bg-green-500/20 rounded-xl px-4 py-3 flex items-center gap-3 border border-green-400/30">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-400/80 mt-0.5">{lang === "id" ? "Semua modul sudah selesai." : "All modules are complete."}</p>
                  </div>
                </div>
              )}
            </div>
